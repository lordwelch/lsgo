...
func (b *Blocks) initR(f *Frame, num int, src io.Reader) (chan []byte, error) {
	...
	if num == 1 /*|| !f.Descriptor.Flags.BlockIndependence()*/ {
		b.Blocks = nil
		b.Block = NewFrameDataBlock(f)
		return nil, nil
	}
	...
	go func() {
		...
		for b.ErrorR() == nil {
			...
			blocks <- c
			go func() {
				data, err := block.Uncompress(f, size.Get(), false, nil)
				if err != nil {
					b.closeR(err)
				} else {
					c <- data
...
func (b *FrameDataBlock) Uncompress(f *Frame, dst []byte, sum bool, window *[]byte) ([]byte, error) {
	if !f.Descriptor.Flags.BlockIndependence() && window == nil {
		return nil, fmt.Errorf("%w: an allocated window of size 0 must be passed in for block independent decompression", lz4errors.ErrInternalUnhandledState)
	}
	if b.Size.Uncompressed() {
		n := copy(dst, b.data)
		dst = dst[:n]
	} else {
		n, err := lz4block.UncompressBlock(b.data, dst, window)
		if err != nil {
			return nil, err
		}
		dst = dst[:n]
	}
	if !f.Descriptor.Flags.BlockIndependence() && len(*window) > 64*1024 {
		copy((*window)[:64*1024], (*window)[len(*window)-64*1024:])
